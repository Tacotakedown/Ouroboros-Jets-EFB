...
export const NgMap = () => {
  ...
  const handleSourceButtonClick = (source: NavigraphRasterSource) => {
    // Your logic for handling source button click
  }

  const handleFAASourceButtonClick = (source: FAASource) => {
    // Your logic for handling FAA source button click
  }

  const handleThemeButtonClick = (theme: NavigraphTheme) => {
  ...
  return (
    <div>
      ...
      <div className="faa-sources">
        ...
        <button onClick={() => handleFAASourceButtonClick('IFR LOW')}>FAA Source 2</button>
        {/* Add more FAA source buttons as needed */}
      </div>
      <div className="themes">
        <button onClick={() => handleThemeButtonClick('DAY')}>day</button>
        <button onClick={() => handleThemeButtonClick('NIGHT')}>night</button>
