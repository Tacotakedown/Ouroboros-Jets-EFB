...
export const MetarProvider: React.FC<T_MetarProviderProps> = (props: T_MetarProviderProps): JSX.Element => {
  ...
  const calculateDensityAltitude = (
    ...
    if (altimeterSettingInHg === undefined || elevationFeet === undefined || temperatureCelsius === undefined)
      return null
  ...
  const weatherConditions = props.parsedMetar.weatherConditions
  ...
  return (
    <div className="metar-wrapper">
      ...
      {Array.isArray(clouds) && clouds.length > 0 ? (
        <MetarOrganizer
          field=" Clouds (AGL):"
          text={
            <div>
              {props.parsedMetar?.clouds[props.parsedMetar.clouds.length - 1]?.quantity}{' '}
              {props.parsedMetar?.clouds[props.parsedMetar.clouds.length - 1]?.height}&rsquo;
            </div>
          }
        />
      ) : (
      ...
      {Array.isArray(weatherConditions) && weatherConditions.length > 0 ? (
        <MetarOrganizer
          field=" Weather:"
          colorOvrd="#ff0066"
          text={
            <div>
              {props.parsedMetar.weatherConditions.map((c, index: number) => {
                return (
      ...
      />
      <MetarOrganizer field="Humidity: " text={<div>{formattedHumidity}</div>} />
      <MetarOrganizer field="Density Altitude:" text={<div> {DA}&rsquo;</div>} />
    </div>
  )
}
